...
class ProfileServer(commands.Cog):
    ...
    async def Profile(self, interaction: Interaction, user:nextcord.Member):
        embed=nextcord.Embed(title="Informacion del usuario", color=0x1FD3F3)
        ...
        embed.add_field(name='Se unio', value=user.joined_at.__format__('%d/%m/%Y, %H:%M:%S'), inline=True)
        embed.add_field(name='Rol mas alto', value=user.roles[-2], inline=True)
